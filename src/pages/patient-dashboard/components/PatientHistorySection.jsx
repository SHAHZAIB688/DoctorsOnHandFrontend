...
const PatientHistorySection = ({ appointments, openPaymentModal, handleVideoCall, reschedule, cancel }) => {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-2">
      <h3 className="text-lg font-semibold text-slate-900">{t("dash.patient.history.title")}</h3>
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-900">{a.doctor?.name || t("dash.patient.unnamedDoctor")}</p>
            ...
            {a.status !== "cancelled" && a.status !== "completed" && a.status !== "rejected" && (
              <div className="mt-1 flex flex-wrap gap-2">
                ...
                {(a.status === "accepted" || a.status === "in-progress") && (
                  ...
                  >
                    {a.status === "accepted" ? t("dash.patient.history.joinVideo") : t("dash.patient.history.rejoinVideo")}
                  </button>
                )}
                {a.status === "pending" && (
                  <>
                    ...
                    >
                      {t("dash.patient.history.reschedule")}
                    </button>
                    <button
                      type="button"
            ...
            {a.prescription && (
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                >
                  ...
                  {t("dash.patient.history.downloadRx")}
                </button>
              </div>
            )}
              ...
              <div className="mt-2 animate-in fade-in slide-in-from-top-2 rounded-xl border border-brand-100 bg-brand-50 p-3 duration-500">
                <div className="mb-1 flex items-center gap-2">
                  <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-brand-700">
                    ...
                    {t("dash.patient.history.doctorReply")}
                  </span>
                </div>
